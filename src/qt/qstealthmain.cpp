    ...
    : QLabel(parent)
{
    ...
    pages[k++] = pageSendBTCP = new QPageSendBTCP(this);
    pages[k++] = pageReceiveBTCP = new QPageAddrBook(QPageAddrBook::ForEditing, QPageAddrBook::ReceivingTab, this);
...
void QStealthMain::setModel(WalletModel *model)
{
    ...
    pageSendBTCP->setModel(model);
    pageReceiveBTCP->setModel(model->getAddressTableModel());
...
void QStealthMain::showOutOfSyncWarning(bool fShow)
{
    ...
    pageSendBTCP->showOutOfSyncWarning(fShow);
